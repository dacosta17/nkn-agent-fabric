...
import { verifyIdentityBindingProof } from './agent-trust.js';
...
function decodeAbiString(hex) {
  ...
  if (offset < 0 || offset + 32 > bytes.length) throw new Error('invalid ABI string offset');
...
export function validateRegistrationFile(registration, { expectedRegistry, expectedAgentId } = {}) {
  ...
  if (!registration.name || !registration.description || !Array.isArray(registration.services)) return { valid: false, reason: 'missing-registration-fields' };
...
export function buildRegistrationFile({ name, description, nknAddress, agentRegistry, agentId, capabilities = [], supportedTrust = ['crypto-economic'] }) {
...
export function verifyExternalAgentAdmission({ registration, agentRegistry, agentId, manifest, transportSource, operatorRegistry, verifyOperatorMembership }) {
  ...
  if (operatorRegistry && verifyOperatorMembership) {
    const membership = verifyOperatorMembership({ registry: operatorRegistry, operatorId: manifest.operatorId, nknAddress: transportSource });
