...
export function createIdentity() {
  ...
  return { publicKey, privateKey, publicKeyB64: b64(publicKey.export({ type: 'spki', format: 'der' })) };
}
...
export function createIdentityBindingProof({ nknAddress, identity }) {
  ...
  const binding = { version: 1, type: 'nkn-identity-binding.v1', nknAddress, applicationPublicKey: identity.publicKeyB64 };
...
export function verifyIdentityBindingProof(proof, { transportSource } = {}) {
  ...
  } catch { return { valid: false, reason: 'binding-verification-error' }; }
}
export function signManifest({ nknAddress, identity, capabilities, endpointClass = 'nkn', ttlMs = DEFAULT_MANIFEST_TTL_MS, identityBinding = createIdentityBindingProof({ nknAddress, identity }) }) {
  if (!nknAddress) throw new Error('nknAddress is required');
  if (identityBinding.nknAddress !== nknAddress || identityBinding.applicationPublicKey !== identity.publicKeyB64) throw new Error('identity binding does not match manifest identity');
  const createdAt = Date.now();
  const manifest = { version: 1, type: 'agent-capability.v1', agentId: nknAddress, endpointClass, publicKey: identity.publicKeyB64, capabilities: [...new Set(capabilities)].sort(), createdAt, expiresAt: createdAt + ttlMs, nonce: randomUUID(), identityBinding };
...
export function verifyManifest(manifest, { now = Date.now(), transportSource, requireIdentityBinding = true } = {}) {
  ...
  if (requireIdentityBinding) {
    ...
    if (manifest.identityBinding.applicationPublicKey !== manifest.publicKey || manifest.identityBinding.nknAddress !== manifest.agentId) return { valid: false, reason: 'identity-binding-manifest-mismatch' };
  }
  try {
  ...
  } catch { return { valid: false, reason: 'signature-verification-error' }; }
}
