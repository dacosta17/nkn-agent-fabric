...
async function createAgent(source) {
  ...
  const transport = await createNknTransport({ identifier: `market-agent-${source}-${process.pid}-${randomUUID().slice(0, 6)}`, numSubClients: 2 });
  ...
  transport.onMessage(async ({ src, payload }) => {
    try {
      ...
      if (env.payload?.task?.type === 'execute.v1') {
        ...
        const taskDigest = digest(env.payload.task.spec);
        const resultDigest = digest(result);
        const attestation = createAttestation({ requestId: env.requestId, agentId: transport.addr, taskDigest, resultDigest, source, capturedAt: result.capturedAt, identity });
...
async function rpc(client, target, task) {
  ...
  return { requestId, response: typeof response === 'string' ? JSON.parse(response) : response };
}

async function main() {
  const coordinator = await createNknTransport({ identifier: `market-coordinator-${process.pid}-${randomUUID().slice(0, 6)}`, numSubClients: 3 });
  const agents = await Promise.all(sources.map(createAgent));
  ...
  try {
    ...
    for (const entry of selected) {
      const { response, requestId } = await rpc(coordinator, entry.agent.transport.addr, { type: 'execute.v1', spec: TASK });
      const result = response?.payload?.result;
      const attestation = response?.payload?.evidence?.attestation;
      const verification = verifyAttestation(attestation, entry.agent.manifest);
      assert.equal(verification.valid, true);
      assert.equal(attestation.taskDigest, digest(TASK));
      assert.equal(attestation.resultDigest, digest(result));
      executions.push({ agent: entry.agent.transport.addr, source: entry.agent.source, quote: entry.quote.price, result, attestation, requestId });
    }
    ...
    if (adversary) {
      ...
      const deviation = Math.abs(adversary.result.price - median) / median;
      assert.ok(deviation > 0.2, 'adversary must materially disagree');
      reputation.record(adversary.agent, 'conflict');
    }
    for (const agent of honest) reputation.record(agent.agent, 'success');

    const report = {
      phase: 'attested-agent-market',
      task: TASK,
      ...
      executions: executions.map((x) => ({ source: x.source, price: x.result.price, attestationVerified: true })),
