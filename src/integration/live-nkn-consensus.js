...
function quorumDecision(observations, now = Date.now()) {
  ...
  const stale = observations.filter((o) => Number.isInteger(o.capturedAt) && now - o.capturedAt > MAX_OBSERVATION_AGE_MS).map((o) => o.source);
  ...
  for (const item of valid) {
    ...
    if (!group) { group = { anchor: item, members: [] }; groups.push(group); }
  ...
  return { quorum: true, sourceCount: winner.members.length, price: Number((winner.members.reduce((sum, item) => sum + item.price, 0) / winner.members.length).toFixed(8)), sources: winner.members.map((item) => item.source), outliers: valid.filter((item) => !winner.members.includes(item)).map((item) => item.source), stale, invalid, groups };
}
...
async function observeFromSource(source) {
  if (source === 'coingecko') {
    ...
    url.searchParams.set('ids', 'nkn'); url.searchParams.set('vs_currencies', 'usd');
  ...
  if (source === 'adversary') return { source, venue: 'Byzantine-Injected', price: 0.12345678, capturedAt: Date.now() };
  throw new Error(`unknown source ${source}`);
}

async function startWorker(source) {
  const transport = await createNknTransport({ identifier: `integration-worker-${source}-${process.pid}-${randomUUID().slice(0, 8)}`, numSubClients: 2 });
  ...
  transport.onMessage(async ({ src, payload }) => {
    try {
      ...
      if (env.payload?.task?.type === 'price-observation.v1') {
        let result; let degraded = false; let recoveryReason = null;
        try { result = await observeFromSource(source); lastObservation = result; }
        catch (error) {
          const cacheAge = lastObservation ? Date.now() - lastObservation.capturedAt : Number.POSITIVE_INFINITY;
          if (lastObservation && cacheAge <= MAX_OBSERVATION_AGE_MS) { result = { ...lastObservation, degraded: true, recoveryReason: error.message }; degraded = true; recoveryReason = error.message; }
          else throw error;
        }
      ...
      if (env.payload?.task?.type === 'ping.v1') return JSON.stringify(createResponse({ requestId: env.requestId, sender: transport.addr, recipient: src, result: { ok: true, worker: source, at: Date.now() }, evidence: { source, digest: digest({ ok: true, worker: source }) } }));
      throw new Error('unsupported task type');
    } catch (error) { return JSON.stringify({ error: error.message, source }); }
  });
...
async function sessionSmokeTest(coordinator, worker) {
  ...
  const serverSession = await Promise.race([accepted, new Promise((_, reject) => setTimeout(() => reject(new Error('session accept timeout')), RESPONSE_TIMEOUT_MS))]);
...
async function centralHttpBaseline(sampleCount) {
  const server = http.createServer((req, res) => {
    if (req.url === '/ping') { res.writeHead(200, { 'content-type': 'application/json' }); res.end(JSON.stringify({ ok: true, at: Date.now() })); return; }
    res.writeHead(404).end();
  });
  await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
  const { port } = server.address(); const timings = [];
  try {
    for (let i = 0; i < sampleCount; i += 1) {
      const started = performance.now(); const response = await fetch(`http://127.0.0.1:${port}/ping`);
...
async function main() {
  ...
  const metrics = { protocol: { packet: false, session: false }, consensusRounds: [], nknRttMs: [], centralRttMs: [], resilience: { adversaryRejected: false, quorumSurvivedAdversaryFailure: false, quorumFailureDetected: false } };
  try {
