...
func TestSharedCanonicalVectors(t *testing.T) {
	for _, name := range []string{"protocol-v1-canonical.json", "protocol-v1-unicode.json"} {
		t.Run(name, func(t *testing.T) {
			...
			if string(canonical) != vector.Canonical {
				t.Fatalf("canonical mismatch: got %s want %s", canonical, vector.Canonical)
			}
			...
			if digest != vector.SHA256 {
				t.Fatalf("digest mismatch: got %s want %s", digest, vector.SHA256)
			}
		})
	}
}
